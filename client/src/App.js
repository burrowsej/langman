...
class App extends Component {
  ...
  startGame(nameValue, langValue) {
    axios
      .post(APIURL + "api/games", { username: nameValue, language: langValue })
      .then((response) => {
        ...
        if (response.data.message === "success") {
          ...
          // TODO: re-engineer the api to avoid needing two requests here!
          axios
            .get(APIURL + `api/games/${gameId}`)
            .then((response2) => {
              this.setState({
  ...
  render() {
    ...
    if (gameStatus === "logged out") {
      ...
      const playAgain = this.this.playAgain;
      const quitGame = this.quitGame;
      screen = <LoseScreen
        usage={usage}
        blanks={secretWord}
      ...
      return <div>Unexpected {gameStatus}</div>;
    }
    return (
      <>
        <GlobalStyle />
