...
class Banner extends Component {
  render() {
    ...
    return (
      <div>
        <TitleFontDiv>Lang-man</TitleFontDiv>
        {full && <SubTitleDiv>play multilingual hangman</SubTitleDiv>}
      </div>
    );
  }
}
...
class ResultBanner extends Component {
  render() {
    ...
    return <BlockFontDiv>{winResult ? "You Won!" : "You Lost"}</BlockFontDiv>;
  }
}
...
function prepareUsage(usage, blanks, showBlanks) {
  ...
  return (
    <p>
      {beforeBlanks}
      <InUsageSpan showBlanks={showBlanks}>{newBlanks}</InUsageSpan>
      {afterBlanks}
    </p>
  );
}
