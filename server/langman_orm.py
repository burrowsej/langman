...
class Usage(base_usage):
    '''Table ``usages`` with fields:
        ...
        * ``secret_word`` - Word to be guessed (length <= 25)
...
class User(base_games):
    ...
    * ``by_lang`` - JSON string storing game counts by language
    * ``first_time`` - DateTime indicating when first game was played
    * ``total_time`` - TimeDelta of total time wiht active games
...
class Game(base_games):
    ...
    * ``usage_id`` - Integer index udage in ``usages`` table
